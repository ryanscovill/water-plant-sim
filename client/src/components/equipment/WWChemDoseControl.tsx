...
interface WWChemDoseControlProps {
  tagId: string;
...
export function WWChemDoseControl({ tagId, currentSetpoint, currentActual, label, unit, min, max, step = 0.1, onDirtyChange }: WWChemDoseControlProps) {
  ...
  const apply = () => {
    getWWEngine().applyControl('setpoint', { tagId, value });
  };
